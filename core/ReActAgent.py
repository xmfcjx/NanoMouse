...
Action Input: 无"""
    ...
    def _base_convert(self, query: str) -> str:
        try:
            match = re.match(r'([\da-fA-F]+)\s*(to|转)\s*(hex|dec|bin|oct)', query.lower())
            if match:
                val_str = match.group(1)
                target = match.group(3)
                
                # 🎯 修复：智能判断源进制
                source_base = 10 # 默认当作十进制
                if val_str.startswith('0b') or (all(c in '01' for c in val_str) and len(val_str) > 1 and val_str != '0'):
                    source_base = 2
                elif val_str.startswith('0o'):
                    source_base = 8
                elif val_str.startswith('0x') or (all(c.lower() in '0123456789abcdef' for c in val_str) and len(val_str) > 1 and any(c.lower() in 'abcdef' for c in val_str)):
                    source_base = 16
