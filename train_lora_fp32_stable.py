...
def load_jsonl_samples(data_path, max_samples=None):
    samples = []
...
def load_training_data(
    ...
    train_samples = load_jsonl_samples(data_path, max_samples)
    if eval_data_path:
        if not os.path.exists(eval_data_path):
            print(f"[Warning] 验证数据不存在: {eval_data_path}")
            return None, None
        val_samples = load_jsonl_samples(eval_data_path)
        print(f"[Data] 使用独立验证集: {eval_data_path}")
    else:
        ...
        print("[Data] 未提供独立验证集，从训练数据固定切分 %.1f%%" % (val_ratio * 100))
...
def train_lora(
    ...
    print("\n[4/5] 准备训练数据...")
    train_samples, val_samples = load_training_data(data_path, eval_data_path)
...
def main():
    ...
    parser.add_argument("--eval_data_path", type=str, default=None, help="独立验证数据路径")
    ...
    if args.local:
        ...
        train_lora(config, args.model_path, args.data_path, args.eval_data_path, args.output_dir)
