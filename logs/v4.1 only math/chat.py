"""
NanoChat-Lab 主程序 (v4.1 - 混合检索 + ReAct 数学推理版) 
新增 BM25 关键词检索，与向量检索互补
新增 ReAct Agent 处理数学计算问题
""" 
...
def is_math_question(text: str) -> bool:
    """简单判断用户输入是否为数学计算类问题"""
    if re.search(r'\d', text) and re.search(r'[\+\-\*\/\×\÷]|等于|计算|算|多少', text):
        return True
    return False

def solve_math_directly(text: str) -> str:
    """尝试用 sympy 直接解方程，不需要 LLM 参与"""
    try:
        import sympy
        eq_match = re.search(r'(.+?)\s*=\s*(.+)', text)
        if not eq_match:
            return ""  # 不是标准方程格式，交给 LLM
        ...
        x = sympy.Symbol('x')
        # 把人类写法转成 Python 表达式
        left_py = left.replace('^', '**')
        right_py = right.replace('^', '**')
        
        equation = sympy.Eq(sympy.sympify(left_py), sympy.sympify(right_py))
        ...
        return ""  # 解析失败，返回空字符串表示交给 Agent 处理
...
def get_response(query, history=None):
    ...
    #  数学题处理：sympy 先算，算不了再丢给 Agent
    if is_math_question(query):
        direct_answer = solve_math_directly(query)
        if direct_answer:  # sympy 成功解出答案
            return direct_answer, "math", "calculator"
        
        # sympy 解不出来，走 ReAct Agent 兜底
        agent_response = agent.run(query)
        #  清洗 Agent 输出：尝试提取最后一个数字作为最终答案
        numbers = re.findall(r"[-+]?\d*\.?\d+", agent_response)
        if numbers:
            clean_answer = numbers[-1] 
        else:
            clean_answer = agent_response.replace("Final Answer:", "").strip()
        return clean_answer, "math", "calculator"
...
# =========================
def main():
    print("NanoChat (v4.1 - Hybrid Retrieval + ReAct Math)") 
