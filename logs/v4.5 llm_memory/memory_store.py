...
class MemoryStore:
    ...
    def _extract_with_llm(self, query: str) -> dict:
        prompt = f"""Is this message about user's personal info? Answer with JSON only.

Message: "{query}"

If user is TELLING about themselves: {{"action": "set", "key": "info_type", "value": "info"}}
If user is ASKING about themselves: {{"action": "get", "key": "what"}}
